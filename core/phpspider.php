...
class phpspider
{
     ...
     */
    public function add_url($url = null , $options = array(), $depth = 0)
    {
        // 投递状态
        $status = false;
    ...
    public function start()
    {
        ...
        {
            log::no_log("uuuuuuuuuuuuuuuuuuuuuuse redis");
            self::$use_redis = true;
    ...
    public function collect_page()
    {
        ...
        log::no_log("Find pages: {$get_collect_url_num} ");
        
        $queue_lsize = $this->queue_lsize();
        log::no_log("Waiting for collect pages: {$queue_lsize} ");
        
        $get_collected_url_num = $this->get_collected_url_num();
        log::no_log("Collected pages: {$get_collected_url_num} ");
        
        // 多任务的时候输出爬虫序号
        if (self::$tasknum > 1)
        {
            log::no_log("Current task id: ".self::$taskid);
        }
        ...
        $spider_time_run = util::time2second(intval(microtime(true) - self::$time_start));
        log::no_log("Spider running in {$spider_time_run}");
    ...
    {
        log::no_log("lein function fill_url: url: ".$url." ;collect_url: ".$collect_url);
        $url = trim($url);
        ...
        log::no_log("lein function fill_url: base_url_path before: ".$base_url_path);
        $base_url_path = preg_replace("/\/([^\/]*)\.(.*)$/","/",$base_url_path);
        $base_url_path = preg_replace("/\/$/",'',$base_url_path);
        log::no_log("lein function fill_url: base_url_path after: ".$base_url_path);
        ...
        else
        {
            ...
            {
                log::no_log("lein function fill_url  是否走到了 else。。。 是");
        ...
        log::no_log("lein function fill_url: base_url_path final: ".$url);
        return $url;
    }
    ...
    public function get_html_fields($html, $url, $page)
    {
        ...
        if (!empty($fields))
        {
            ...
            if (isset($fields) && is_array($fields))
            {
                ...
                log::no_log("Result[{$fields_num}]: ".$fields_str);
    ...
    public function get_fields($confs, $html, $url, $page)
    {
        ...
        foreach ($confs as $conf)
        {
            ...
            if (!empty($conf['selector']))
            {
                ...
                {
                    log::no_log("-------------配置了source_type 为 attached_url-------------");
                    log::no_log("-------------attached_url原始代码： ".$conf['attached_url']);
                    log::no_log("-------------总共field的字段： ".json_encode($fields));
                    // 取出上个field的内容作为连接, 内容分页是不进队列直接下载网页的
                    if (!empty($fields[$conf['attached_url']]))
                    {
                        log::no_log("-------------配置了 attached_url-------------");
                        ...
                        {
                            $return = call_user_func($this->on_download_attached_page, $html, $collect_url, $this);
                            if (isset($return))
                            {
                                $html = $return;
                            }
                ...
                log::no_log("xxx 判断children钱的vaules信息： ".json_encode($values));
                // field不为空而且存在子配置
                if (isset($values) && !empty($conf['children']))
                {
                    ...
                    foreach ($values as $child_html)
                    {
                        ...
                        log::no_log("------ array children---------: ".json_encode($conf['children']));
                        // 递归调用本方法, 所以多少子项目都支持
    ...
    {
        log::no_log("----my log----- ".self::$use_redis."----".self::$save_running_state."-------");
        if (!self::$use_redis || self::$save_running_state)
        {
            log::no_log("----my log----- NOT check_cache--------");
            return false;
        }
        ...
        $keys = queue::keys("*");
        $count = count($keys);
        log::no_log("----my log----- check_cache--------");
        if ($count != 0)
        {
            log::no_log("----my log----- check_cache----count != 0----");
